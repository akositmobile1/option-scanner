...
watchlist = [
    t.strip().upper() for t in watchlist_input.split(",") if t.strip()
]
...
def fetch_and_analyze_data(tickers):
  ...
  for ticker in tickers:
    try:
      ...
      sma20 = df["20_SMA"].iloc[-1]
      sma50 = df["50_SMA"].iloc[-1]
      rsi = df["RSI"].iloc[-1]
      ...
      volatility_est = daily_pct_change.tail(14).mean()
...
if scanner_df.empty:
  ...
  top_pick = (
      scanner_df.iloc[0]["Ticker"] if not scanner_df.empty else "None"
  )
  ...
  st.dataframe(
      ...
      column_config={
          "Score": st.column_config.ProgressColumn(
              "Match Score", min_value=0, max_value=100, format="%d pts"
          ),
          "Price": st.column_config.NumberColumn(
              "Stock Price", format="$%.2f"
          ),
          "Est_Strike": st.column_config.NumberColumn(
              "Est. Strike", format="$%.2f"
          ),
          "Est_Premium": st.column_config.NumberColumn(
              "Est. Premium", format="$%.2f"
          ),
          "Annualized_ROC_%": st.column_config.NumberColumn(
              "Ann. Yield", format="%.1f%%"
          ),
          "Dist_20SMA_%": st.column_config.NumberColumn(
              "20-SMA Dist", format="%.1f%%"
          ),
  ...
  c3.metric(
      "Max Contracts",
      f"{ticker_row['Contracts']} ({ticker_row['Total_Collateral']:,.0f} Collateral)",
  )
  ...
  try:
    ...
    if expirations:
      # Filter for expirations close to ~30-45 DTE
      exp_choice = st.selectbox(
          "Select Live Expiration Date", options=expirations[:6]
      )
